...
let maxSize = 0;
let availableSlot = [];
const utilFunctions = require('./parkingUtilFunc');
...
let createParkingLot = async (noOfLot) => {
    ...
    //const slot_obj = JSON.parse(JSON.stringify(slot));
    //let output = new Array(noOfLot).fill().map((e, i) => { return {...slot, no: i}});
    
    // const output = [...availableSlot];
...
let leave = async (slot) => {
  ...
  if (maxSize === 0) {
    ...
    if (await utilFunctions.search(slot, 'slot', Car)) {
        ...
        let Receipt = await utilFunctions.receipt(slot, 'receipt', Car);
        console.log('Receipt',Receipt)
        // Add a the number back into slot 
        availableSlot.push(slot);
        availableSlot.sort();
        return `Slot  numbmer ${slot} is free and ${Receipt}`;
